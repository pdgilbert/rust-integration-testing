//! Continuously read temperature from AHT10 and with semihosting hprintln.
//!
...
#[entry]
fn main() -> ! {
    hprintln!("AHT aht10-aht30-semi example");
    ...
    //  asm::bkpt();   
    let mut aht = Aht10::new(AHT10_DEFAULT_ADDR, i2c1, delay);
    aht.calibrate().expect("sensor calibrate failed.");
    hprintln!("Sensor started.");

    loop {        
        hprintln!("aht.measure()");
        let (humidity, temperature)  = aht.read().expect("read error").decode();
